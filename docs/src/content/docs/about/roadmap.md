...
| Zero-downtime version upgrades | Opt-in (`spec.prePull: true`) Pre-pull Job pulls new OCI image layers and pre-downloads the Minecraft server jar to the data PVC while the server is still running; rolling update only waits for server startup |

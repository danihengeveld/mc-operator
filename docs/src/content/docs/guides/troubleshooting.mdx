...
- **Helm deployments**: The chart auto-generates certificates and injects the `caBundle` automatically. Try reinstalling the chart to regenerate certificates:

  ```bash
  helm upgrade --install mc-operator oci://ghcr.io/danihengeveld/charts/mc-operator \
    --namespace mc-operator-system
  ```

- **Kustomize deployments**: Ensure the `caBundle` in the webhook configurations matches the CA that signed the server certificate. Re-export and patch if needed:

  ```bash
  CA_BUNDLE=$(kubectl get secret webhook-cert -n mc-operator-system -o jsonpath='{.data.ca\.crt}')
